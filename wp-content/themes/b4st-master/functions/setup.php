...
if (!function_exists('b4st_contact_menu')) {
    function b4st_contact_menu()
    {
        $page = get_page_by_path('contact');
        $page_ID_current_lang = pll_get_post($page->ID);
        $link = empty($page_ID_current_lang) ? get_permalink($page->ID) : get_permalink($page_ID_current_lang);
        $html = '<div class="navbar-contact"><a href="' . $link . '" class="btn-contact"><i class="fas fa-envelope"></i></a>';
        $html .= '<p><a class="link-contact" href="' . $link . '">';
        $html .= pll__('contact us');
        $html .= '</a></p></div>';
        return $html;
    }
}
add_filter('contact_menu', 'b4st_contact_menu');